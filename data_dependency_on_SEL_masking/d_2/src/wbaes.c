...
int shiftindex[]={0, 13, 10, 7, 4, 1, 14, 11, 8, 5, 2, 15, 12, 9, 6, 3};
...
shares *attack_evaland(shares *x, shares *y, u8 ptx)
{
    ...
    u8 ra[3][N + 1], rb[N + 1][N + 1], u = 0, v = 0;
    shares *s = (shares *)malloc(sizeof(shares));
    u8 x_cooperands[3 + N] = {0};
    u8 y_cooperands[3 + N] = {0};
    x = refresh(x);
    y = refresh(y);
    for(i = 0; i <= 2; i++)
    {
        ...
        for(j = i + 1; j <= N; j++)
        {
            if(i == 0) 
            {
                rb[j][0] = (x->a[0] & ((x->a[2] & ((x->a[1] & y->b[j]) ^ (ra[0][j] & y->a[0]))) ^ (ra[1][j] & v & y->a[1]))) ^ \
                    ...
                    (x->a[2] & y->a[0] & ((ra[0][j] & x->a[0]) ^ (ra[1][j] & y->a[1]))) ^ (u & v & ra[0][j]);
                //--
                x_cooperands[0] ^= ((x->a[2] & ((x->a[1] & y->b[j]) ^ (ra[0][j] & y->a[0]))) ^ (ra[1][j] & v & y->a[1])) ^ (y->a[1] & ((ra[1][j] & x->a[2] & y->a[0]) ^ (ra[2][j] & x->a[1] & y->a[2]))) ^ ra[0][j]; //x->a[0]
                x_cooperands[1] ^= y->b[j] ^ (ra[2][j] & y->a[2]) ^ (ra[0][j] & y->a[2] & (v ^ (x->a[2] & y->a[0]))) ^ (ra[2][j] & y->a[2]);  //x->a[1]
                x_cooperands[2] ^= ((x->a[1] & y->b[j]) ^ (ra[0][j] & y->a[0])) ^ ra[1][j] ^ (ra[0][j] & u) ^ (ra[1][j] & y->a[0]) ^ (y->a[0] & ((ra[0][j] & x->a[0]) ^ (ra[1][j] & y->a[1])));  //x->a[2]
                x_cooperands[2 + j] ^= y->a[2];            //x->b[j]

                y_cooperands[0] ^= ra[0][j] ^ ((y->a[1] & ((y->a[2] & x->b[j]) ^ (ra[1][j] & x->a[2]))) ^ (ra[0][j] & u & x->a[2])) ^ (ra[1][j] & x->a[2]) ^ (x->a[2] & ((ra[0][j] & x->a[0]) ^ (ra[1][j] & y->a[1]))); //y->a[0]
                y_cooperands[1] ^= (ra[1][j] & v) ^ ((y->a[2] & x->b[j]) ^ (ra[1][j] & x->a[2])) ^ (x->a[0] & ((ra[1][j] & x->a[2] & y->a[0]) ^ (ra[2][j] & x->a[1] & y->a[2]))) ^ ra[1][j];  //y->a[1]
                y_cooperands[1] ^= x->b[j] ^ (ra[2][j] & x->a[1]) ^ (ra[0][j] & x->a[1] & (v ^ (x->a[2] & y->a[0]))) ^ (ra[2][j] & x->a[1]); // y->a[2]
                y_cooperands[2 + j] ^= x->a[1];            //y->b[j]
                //--
            }
            else
            {
                ...
                rb[j][i] = rb[i][j] ^ (x->b[i] & y->b[j]) ^ (x->b[j] & y->b[i]);
                //--
                x_cooperands[2 + i] ^= y->b[j];  //x->b[i]
                x_cooperands[2 + j] ^= y->b[i];  //x->b[j]

                y_cooperands[2 + j] ^= x->b[i];  //y->b[j]
                y_cooperands[2 + i] ^= x->b[j];  //y->b[i]
                //--
            }
        }
    }
    for(i = 1; i <= N; i++)
    {
        s->b[i] = x->b[i] & y->b[i];
        //--
        x_cooperands[2 + i] ^= y->b[i];  //x->b[i]
        y_cooperands[2 + i] ^= x->b[i];  //y->b[i]
        //--
        for(j = 0; j <= N; j++)
        {
            if(j != i) s->b[i] ^= rb[i][j];
...
void attack_wbaes(int byteth)
{
    ...
    for(i = 0; i < 64; i++)
    {
        if(CDCA(byteth, i)) count++;
    }
    printf("correct: %d/64\n\n", count);
//     FILE *fpWrite = fopen("DDHO-DCA_d2_1st_bs_round.txt", "a");
//     fprintf(fpWrite, "correct: %d/64\n\n", count);
//     fclose(fpWrite);
}
double score[256][SAMNUM] = {0.0};
int CDCA(int byteth, int bit)
{
    int i, j, k;
    ...
    printf("byte: %d | bit: %d | key guess: %02x | Cor: %f ==> correct key: %f\n", byteth, bit, key_guess, key_guess_value, kmax[key[byteth]]);
    // FILE *fpWrite = fopen("DDHO-DCA_d2_1st_round_t29_savar_scores.txt", "a"); // DDHO-DCA_d2_1st_round_t29_savar_scores DDHO-DCA_d2_1st_round_t29_scores
    // for(k = 0; k < 256; k++)
    // {
    //     for(j = 0; j < index; j++)
    //     {
    ...
    // }
    // FILE *fpWrite = fopen("DDHO-DCA_d2_1st_bs_round.txt", "a");
    // fprintf(fpWrite, "byte: %d | bit: %d | key guess: %02x | Cor: %f ==> correct key: %f\n", byteth, bit, key_guess, key_guess_value, kmax[key[byteth]]);
    // fclose(fpWrite);
    // if(key[byteth] == key_guess) return 1;
    // else return 0;
}